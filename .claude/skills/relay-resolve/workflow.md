...
0. Prerequisite check: Read the target item file(s) and verify
   ...
   - **If the target carries a promoted-feature header** (front-matter contains `*Promoted from:*`): additionally verify promoted-feature closure readiness. Read the applied closure tier per the decision table below.
     ...
     **12-4 layering boundary**: read `*Closure Tier Applied:*` from the front-matter; if present, that is the FINAL applied tier. If absent, fall back to `*Closure Tier Baseline:*`. This permissive forward-reference allows 12-4's waiver logic to layer cleanly. 12-3 sets baseline only; 12-4 sets applied.
     ...
     **Tier-2 disposition options** (parallel to grouped-run dispositions):
     ...
     - **Idempotency-prep note**: this gate determines whether /relay-resolve step 4's bidirectional closure branch (sub-edit 5b) will fire. The actual back-update with idempotency guard happens in step 4. (The read order `*Closure Tier Applied:*` first, falling back to baseline, applies here too — the read-order is the same as in the 12-4 layering boundary bullet above.)
...
4. Archive each resolved item and its notebook:
   ...
   - **Promoted-feature bidirectional closure** (only when this resolution is closing a promoted feature — target carries `*Promoted from:*` in its front-matter):
       ...
       - **Step 4e — Run-leader implementation doc cross-reference**: in the feature's implementation doc (created in step 2), add a `## Promoted Feature Closure` section listing: `*Promoted from:*` source, `*Promotion Class:* lightweight | broad`, `*Closure Tier Baseline:* tier-1 | tier-2`, the applied tier (`*Closure Tier Applied:*`, may equal baseline or downgraded by 12-4 waiver if present), Promoted Feature Coverage diff verification result, and (if Tier 2) Tier 2 disposition table from Step 0's Promoted Feature Closure block.
