...
1. **Edge cases for code_3_review.md section 2**:
   ...
   edge case placeholders in code_3_review.md section 2.

2. **Regression check commands for code_4_verify.md section 4**:
   ...
   generic regression check bullets in code_4_verify.md section 4.
...
3. **Notebook patterns for code_5_notebook.md**:
   ...
   sections 3 (seed data), 8 (cleanup), and the Guidelines section.
...
1. Read every .md file in docs/issues/ and docs/features/
   - Exclude brainstorm files (*_brainstorm.md) from the item list — these
     are grouping/exploration files, not actionable work items. Note them
     in relay-status.md under a separate "Active Brainstorms" section if any exist.
...
8. Verify project-specific workflow customizations are still accurate:
   ...
   - code_3_review.md section 2 (edge cases)
   - code_4_verify.md section 4 (regression check commands)
   - code_5_notebook.md sections 3, 8, and Guidelines (seed data, cleanup, async patterns)
...
- Brainstorm files (`*_brainstorm.md`) are tracked separately — they represent in-progress exploration, not items to implement directly
...
3. Check for intra-feature dependencies:
   - Read any brainstorm files (*_brainstorm.md) in docs/features/ — these
     contain a Feature Breakdown table and Development Order section that
     specify the intended build sequence for related features
   - Read the Development Order and Dependencies sections in individual
     feature files — these record which features depend on others
...
6. Produce an ordered implementation plan in docs/relay-ordering.md with:
   ...
   - If a phase contains features from the same brainstorm, note the
     brainstorm file and the intended build order
...
- Brainstorm files specify intra-feature sequencing; this prompt handles cross-cutting prioritization
```
...
5. After creating all issue files, regenerate docs/relay-status.md
...
Output: New issue files in docs/issues/, updated docs/relay-status.md
~~~
...
**Sequence**: **`discovery_2`** → `code_1_analyze` → `code_2_plan` → `code_3_review` → *implement* → `code_4_verify` → `code_5_notebook` → `code_6_resolve`
...
6. After creating the file, tell the user to run prepare_1_scan_and_status
   and prepare_2_generate_ordering to update project status and priorities.
...
**Sequence**: **`feature_1_brainstorm`** → `feature_2_design` → `code_1_analyze` → `code_2_plan` → `code_3_review` → *implement* → `code_4_verify` → `code_5_notebook` → `code_6_resolve`
...
5. Create and maintain a brainstorm file in docs/features/:
   ...
   - Structure:
     ...
     *Created: [DATE]*
     *Status: BRAINSTORMING | READY FOR DESIGN*
...
6. When brainstorming feels complete:
   ...
   - Explicitly state: "This brainstorm is ready for feature_2_design.
     Run that prompt next to design each feature in detail."
...
~~~
Design the features from the brainstorm.

1. Read the input file in docs/features/:
   - If a brainstorm file exists (*_brainstorm.md with status READY FOR
     DESIGN), read it for the Feature Breakdown table, decisions, development
     order, and open questions. This is the multi-feature path.
   - If working from a standalone feature file (created by discovery_2 or
     directly), read it and treat it as a single-feature design task —
     skip the brainstorm-specific steps (Feature Breakdown, development
     order between features) and proceed directly to step 2.
...
3. After all feature files are created:
   - If a brainstorm file was the input:
     - Set its status to DESIGN COMPLETE
     - Update the Feature Breakdown table with links to the created files
   - Summarize what was created and the development order
   - Explicitly state: "Run prepare_1_scan_and_status and
     prepare_2_generate_ordering to integrate these into the project
     backlog. Then run code_1_analyze to begin implementation."
...
7. Persist the plan by APPENDING it to each relevant issue/feature file in docs/issues/ or docs/features/.
   Add a horizontal rule separator, then append these sections:
...
5. Produce the final verdict (see structure in step 6).

6. Persist the review by APPENDING it to each relevant issue/feature file in
...
- Edge cases in section 2 should be specific to this project — update them as the codebase evolves
...
~~~
Create a verification notebook for each resolved issue/feature file in this
phase, then RUN every cell and iterate until all cells pass.
...
3. SEED DATA: Ingest realistic test content that exercises the changed code
   path. Use domain-appropriate test data unless the change requires
   different data.
...
- **Issue found** → `discovery_2` (quick file) → `code_1` → `code_2` → `code_3` → implement → `code_4` → `code_5` → `code_6`
- **Feature idea** → `feature_1` (brainstorm) → `feature_2` (design) → `code_1` → `code_2` → `code_3` → implement → `code_4` → `code_5` → `code_6`
- **Small feature** → `discovery_2` (quick file) → `code_1` → `code_2` → ...
